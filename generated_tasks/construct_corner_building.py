...
class ConstructCornerBuilding(Task):
    ...
    def reset(self, env):
        ...
        colors = [utils.COLORS['red'], utils.COLORS['green'], utils.COLORS['blue'],
                  utils.COLORS['orange'], utils.COLORS['yellow']]
        ...
        corner_positions = [(0.55, 0.05, 0.02), (0.45, 0.05, 0.02), (0.45, -0.05, 0.02),
                            (0.55, -0.05, 0.02)]
        ...
        # Add goals
        for i in range(1, 5):
            self.add_goal(objs=[blocks[i]], matches=np.ones((1, 1)), targ_poses=[target_poses[i]],
                          replace=False,
                          rotations=True, metric='pose', params=None, step_max_reward=1 / 6,
                          language_goal=self.lang_template)
        self.add_goal(objs=[blocks[0]], matches=np.ones((1, 1)), targ_poses=[target_poses[0]],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 6,
                      language_goal=self.lang_template)
