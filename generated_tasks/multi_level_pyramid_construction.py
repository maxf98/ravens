...
class MultiLevelPyramidConstruction(Task):
    ...
    def reset(self, env):
        ...
        block_colors = [utils.COLORS['green']] * 3 + [
            utils.COLORS['blue']] * 3  # three green and three blue blocks
        ...
        self.add_goal(objs=blocks[:3], matches=np.ones((3, 3)), targ_poses=targs[:3],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 2,
                      symmetries=[np.pi / 2] * 3,
                      language_goal=self.lang_template.format(blocks="the green blocks",
                                                              row="bottom"))
