...
class ColorCoordinatedSphereOnPalletPyramid(Task):
    ...
    def reset(self, env):
        ...
        for color in colors:
            ...
            targs = [
                (utils.apply(color_objects[color]['pallet'], i), color_objects[color]['pallet'][1])
                for i in block_poses]

            self.add_goal(objs=color_objects[color]['blocks'], matches=np.ones((3, 3)),
                          targ_poses=targs, replace=False,
            ...
            sphere_pose = (0, 0, 0.1)
            targ = (utils.apply(color_objects[color]['pallet'], sphere_pose),
                    color_objects[color]['pallet'][1])

            self.add_goal(objs=[color_objects[color]['sphere']], matches=np.ones((1, 1)),
                          targ_poses=[targ], replace=False,
                          rotations=True, metric='pose', params=None, step_max_reward=1 / 2,
                          symmetries=[np.pi / 2],
                          language_goal=self.lang_template.format(color=color))
