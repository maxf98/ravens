...
class CreatePyramidBlocksAndContainer(Task):
    ...
    def reset(self, env):
        ...
        replace = {'DIM': container_size,
                   'HALF': (container_size[0] / 2, container_size[1] / 2, container_size[2] / 2)}
        ...
        block_colors = [utils.COLORS['red'], utils.COLORS['red'], utils.COLORS['green'],
                        utils.COLORS['green'], utils.COLORS['blue'], utils.COLORS['blue']]
        blocks = []
        for i in range(6):
        ...
        place_pos = [(0, -0.05, 0.03), (0, 0, 0.03), (0, 0.05, 0.03), (0, -0.025, 0.08),
        ...
        self.add_goal(objs=blocks[2:5], matches=np.ones((3, 3)), targ_poses=targs[:3],
                      replace=False,
        ...
        self.add_goal(objs=blocks[:2], matches=np.ones((2, 2)), targ_poses=targs[3:5],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 3,
        ...
        self.add_goal(objs=blocks[5:], matches=np.ones((1, 1)), targ_poses=targs[5:],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 3,
                      symmetries=[np.pi / 2],
