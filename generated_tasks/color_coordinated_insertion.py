...
class ColorCoordinatedInsertion(Task):
    ...
    def reset(self, env):
        ...
        for i in range(len(fixtures)):
            self.add_goal(objs=[fixtures[i]], matches=np.ones((1, 1)), targ_poses=[pallet_pose],
                          replace=False,
                          rotations=True, metric='zone', params=[(pallet_pose, pallet_size)],
                          step_max_reward=1 / len(fixtures),
                          language_goal=self.lang_template)
