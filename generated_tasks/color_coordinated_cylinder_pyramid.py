...
class ColorCoordinatedCylinderPyramid(Task):
    ...
    def reset(self, env):
        ...
        self.add_goal(objs=cylinders[:2], matches=np.ones((2, 2)), targ_poses=targs[:2],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 2,
                      symmetries=[np.pi / 2] * 2,
                      language_goal=self.lang_template.format(
                          cylinder="the red and blue cylinders", row="bottom"))

        # Goal: cylinders are stacked in a pyramid (middle row: green).
        self.add_goal(objs=cylinders[2:3], matches=np.ones((1, 1)), targ_poses=targs[2:3],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 3,
                      symmetries=[np.pi / 2] * 1,
                      language_goal=self.lang_template.format(cylinder="the green cylinder",
                                                              row="middle"))

        # Goal: cylinders are stacked in a pyramid (top row: yellow).
        self.add_goal(objs=cylinders[3:], matches=np.ones((1, 1)), targ_poses=targs[3:],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 4,
                      symmetries=[np.pi / 2] * 1,
                      language_goal=self.lang_template.format(cylinder="the yellow cylinder",
                                                              row="top"))
