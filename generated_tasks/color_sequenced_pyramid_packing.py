...
class ColorSequencedPyramidPacking(Task):
    ...
    def reset(self, env):
        ...
        # Cube colors.
        colors = [
            utils.COLORS['red'], utils.COLORS['green'], utils.COLORS['blue'],
            utils.COLORS['yellow']
        ]
        ...
        targs = [(utils.apply(pallet_pose, i), pallet_pose[1]) for i in place_pos for pallet_pose
                 in pallet_poses]

        # Goal: cubes are sorted by color and stacked in a pyramid in each pallet.
        for i in range(4):
            self.add_goal(objs=objs[i * 3:(i + 1) * 3], matches=np.ones((3, 3)),
                          targ_poses=targs[i * 3:(i + 1) * 3], replace=False,
                          rotations=True, metric='pose', params=None, step_max_reward=1 / 4,
                          symmetries=[np.pi / 2] * 3,
                          language_goal=self.lang_template.format(
                              color=list(utils.COLORS.keys())[i]))
