...
class ColorCoordinatedContainerFill(Task):
    ...
    def reset(self, env):
        ...
        for i in range(4):
            ...
            replace = {'DIM': container_size, 'HALF': (
            container_size[0] / 2, container_size[1] / 2, container_size[2] / 2)}
            container_urdf = self.fill_template(container_template, replace)
            container_id = env.add_object(container_urdf, container_pose, 'fixed', color=colors[i])
            containers.append(container_id)

            language_goal = self.lang_template.format(color=color_names[i])
            self.add_goal(objs=[blocks[i]], matches=np.int32([[1]]), targ_poses=[container_pose],
                          replace=False,
