...
class SortAndAssembleBlockCastle(Task):
    ...
    def reset(self, env):
        ...
        for i in range(12):
            self.add_goal(objs=[blocks[i]], matches=np.ones((1, 3)), targ_poses=zone_poses,
                          replace=False,
        ...
        for i in range(3):
            ...
            for j in range(4):
                self.add_goal(objs=[zone_blocks[j]], matches=np.ones((1, 1)),
                              targ_poses=[targs[j]], replace=False,
                              rotations=True, metric='pose', params=None, step_max_reward=1 / 12,
