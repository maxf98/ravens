...
class ColorCoordinatedBallPlacement(Task):
    ...
    def reset(self, env):
        ...
        for i in range(len(balls)):
            ...
            self.add_goal(objs=[balls[i]], matches=np.ones((1, 1)),
                          targ_poses=[p.getBasePositionAndOrientation(boxes[i])], replace=False,
                          rotations=True, metric='pose', params=None,
                          step_max_reward=1 / len(balls), language_goal=language_goal)
