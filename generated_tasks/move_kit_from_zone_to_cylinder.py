...
class MoveKitFromZoneToCylinder(Task):
    ...
    def reset(self, env):
        ...
        for i in range(n_zones):
            ...
            cylinder_id = env.add_object(cylinder_urdf, cylinder_pose, category='fixed',
                                         color=colors[i])
        ...
        pick_name = color_names[target_idx] + " " + utils.assembling_kit_shapes[
            obj_shapes[target_idx]]
        language_goal = (self.lang_template.format(pick=pick_name, place=color_names[target_idx]))
        self.add_goal(objs=[objects_ids[target_idx]], matches=np.ones((1, 1)),
                      targ_poses=[cylinder_poses[target_idx]], replace=False,
