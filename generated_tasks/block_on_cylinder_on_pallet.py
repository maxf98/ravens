...
class BlockOnCylinderOnPallet(Task):
    ...
    def reset(self, env):
        ...
        self.add_goal(objs=[cylinders[0]], matches=np.ones((1, 1)), targ_poses=[pallet_pose],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 2,
                      language_goal=self.lang_template.format(cylinder_colors[0]))

        # Goal: place the block on top of the cylinder
        language_goal = self.lang_template_2.format(block_colors[0], cylinder_colors[0])
        self.add_goal(objs=[blocks[0]], matches=np.ones((1, 1)), targ_poses=[pallet_pose],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 2,
