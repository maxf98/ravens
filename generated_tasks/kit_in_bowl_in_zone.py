...
class KitInBowlInZone(Task):
    ...
    def reset(self, env):
        ...
        self.add_goal(objs=[bowls[0]], matches=np.ones((1, 1)), targ_poses=[zone_pose],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 2,
                      language_goal=self.lang_template.format(bowl_colors[0]))
        ...
        self.add_goal(objs=[kits[0]], matches=np.ones((1, 1)), targ_poses=[zone_pose],
                      replace=False,
                      rotations=True, metric='pose', params=None, step_max_reward=1 / 2,
