...
class PushPilesIntoLetter(Task):
    ...
    def reset(self, env):
        ...
        target_poses = self.get_target_sample_surface_points(shape, scale, zone_pose,
                                                             num_points=num_blocks)

        # Add goal
        self.add_goal(objs=obj_ids, matches=np.ones((num_blocks, num_blocks)),
                      targ_poses=target_poses, replace=False,
