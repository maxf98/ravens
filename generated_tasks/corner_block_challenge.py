...
class CornerBlockChallenge(Task):
    ...
    def reset(self, env):
        ...
        block_colors = [utils.COLORS['red']] * 4 + [utils.COLORS['green']] * 2 + [
            utils.COLORS['blue']] * 2
        blocks = []
        for i in range(8):
        ...
        for i in range(4):
            self.add_goal(objs=[blocks[i]], matches=np.ones((1, 1)), targ_poses=[corner_poses[0]],
                          replace=False,
                          rotations=True, metric='pose', params=None, step_max_reward=1 / 8,
                          language_goal=self.lang_template)

        for i in range(4, 8):
            self.add_goal(objs=[blocks[i]], matches=np.ones((1, 1)), targ_poses=[corner_poses[1]],
                          replace=False,
                          rotations=True, metric='pose', params=None, step_max_reward=1 / 8,
                          language_goal=self.lang_template)
