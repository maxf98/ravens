    ...
    def reset(self, env):
        ...
        for color in line_colors:
            ...
            replace = {'DIM': line_size,
                       'HALF': (line_size[0] / 2, line_size[1] / 2, line_size[2] / 2),
                       'COLOR': color}
        ...
        for color in line_colors:
            ...
            replace = {'DIM': zone_size,
                       'HALF': (zone_size[0] / 2, zone_size[1] / 2, zone_size[2] / 2),
                       'COLOR': color}
        ...
        for color in block_colors:
            ...
            replace = {'DIM': block_size,
                       'HALF': (block_size[0] / 2, block_size[1] / 2, block_size[2] / 2),
                       'COLOR': color}
        ...
        # Add goals.
        for i in range(3):
            self.add_goal(objs=[block_ids[i]], matches=np.ones((1, 1)), targ_poses=[zone_poses[i]],
                          replace=False,
                          rotations=False, metric='zone', params=[(zone_poses[i], zone_size)],
                          step_max_reward=1 / 3,
