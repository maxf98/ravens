...
class PackingShapes(Task):
    ...
    def reset(self, env):
        ...
        for i in range(num_objects_to_pick):
            ...
            self.add_goal(objs=[objects[i]], matches=np.int32([[1]]), targ_poses=[zone_pose],
                          replace=False,
                          rotations=True, metric='zone', params=[(zone_pose, zone_size)],
                          step_max_reward=1 / num_objects_to_pick,
