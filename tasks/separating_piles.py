...
relevant to the task, another as a distractor."""
    ...
    def reset(self, env):
        ...
        # sample colors
        (zone1_color, zone2_color, block_color), color_names = utils.get_colors(mode=self.mode,
                                                                                n_colors=3)
        ...
        language_goal = self.lang_template.format(block_color=color_names[2],
                                                  square_color=zone_target_color)
        self.add_goal(objs=obj_ids, matches=np.ones((50, 1)), targ_poses=[zone_target],
                      replace=True,
                      rotations=False, metric='zone', params=[(zone_target, zone_size)],
