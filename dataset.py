...
class RavensDataset(Dataset):
    ...
    def __init__(self, path, cfg, n_demos=0, augment=False, random=True):
        ...
        if self.n_demos > 0:
            ...
            if self.random:
                ...
                episodes = list(range(self.n_demos))
            self.set(episodes)
...
class RavensMultiTaskDataset(RavensDataset):
    ...
    def __init__(self, path, cfg, group='multi-all',
        ...
        self.attr_train_task = self.MULTI_TASKS[group][
            'attr_train_task'] if group in self.MULTI_TASKS and 'attr_train_task' in \
                                  self.MULTI_TASKS[group] else None
        ...
        for task in self.tasks:
            ...
            episodes[task] = np.random.choice(range(self.n_demos), min(self.n_demos, n_episodes),
                                              False)
    ...
    def __init__(self, path, cfg, group='multi-all',
        ...
        for task in self.tasks:
            ...
            if task in self.ORIGINAL_NAMES and self.mode == 'train':
                ...
                episodes[task] = np.random.choice(range(n_episodes),
                                                  min(int(self.n_demos * balance_weight),
                                                      n_episodes), False)
            else:
