...
class InsertBlocksToDifferentColorFixture(Task):
    ...
    def __init__(self):
        super().__init__()
        self.max_steps = 20
        self.lang_template = "insert the block with different colors into the fixture"
        self.task_completed_desc = "done inserting blocks."
        self.obj = {}
        self.goal = """Each L-shaped fixture can hold three blocks, suppose the block size is (a,a,a), then in fixture's local coordinate system, the three places that can hold blocks are [(0,0,0),(a,0,0),(0,a,0)]. Fill in all the fixtures which have random position and rotation with blocks with the same color as the fixture"""
    ...
    def reset(self, env):
        ...
        match = np.zeros((3*len(colors), 3*len(colors)))
        for i in range(len(colors)):
            match[3*i:3*i+3, 3*i:3*i+3]=1

        # Goal: each block is in the corresponding color fixture.
        self.add_goal(objs=blocks, matches=match, targ_poses=fixture_poses,
                          replace=False,
